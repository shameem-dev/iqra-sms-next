...
function KpiCard({ label, value, sub, icon: Icon, gradient, badge, badgePositive }: any) {
  return (
    <div className={`relative overflow-hidden rounded-[28px] p-6 ${gradient} shadow-lg`}>
      <div className="absolute -right-6 -top-6 w-32 h-32 rounded-full bg-white/10" />
      <div className="absolute right-6 top-10 w-16 h-16 rounded-full bg-white/8" />
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-5">
          <span className="text-[9px] font-bold uppercase tracking-[.22em] text-white/60">{label}</span>
        ...
        {(sub || badge) && (
          <div className="flex items-center gap-2 mt-3">
            {badge && (
              <span className={`text-[9px] font-bold px-2.5 py-1 rounded-full ${badgePositive ? 'bg-white/25 text-white' : 'bg-black/25 text-white/90'}`}>
                {badge}
              </span>
            )}
            {sub && <span className="text-[10px] text-white/55 font-medium">{sub}</span>}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="py-0.5">
      <div className="flex justify-between items-center mb-2">
        <span className="text-[13px] text-slate-500 font-medium capitalize">{label.replace(/_/g, ' ')}</span>
        <span className="text-[13px] font-bold text-slate-800">{fmtINR(amount)}</span>
      </div>
      <div className="h-[5px] bg-slate-100 rounded-full overflow-hidden">
        <div className={`h-full rounded-full ${colors}`} style={{ width: `${p}%`, transition: 'width 1.2s cubic-bezier(.4,0,.2,1)' }} />
      </div>
    </div>
  );
}
      ...
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <header className="flex items-center justify-between bg-white rounded-[20px] border border-slate-100 shadow-sm px-5 py-3.5">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[17px] font-black text-slate-900 tracking-tight">Iqrah Public School</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={load} className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-500 hover:text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-3.5 py-1.5 rounded-xl transition-all">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KpiCard label="Students" value={String(data.students)} sub="enrolled this term"
            icon={Users} gradient="bg-gradient-to-br from-violet-500 via-purple-500 to-indigo-600" />
          <KpiCard label="Revenue" value={fmtINR(data.income)} badge="+12.4%" badgePositive sub="vs last month"
            icon={TrendingUp} gradient="bg-gradient-to-br from-emerald-400 via-teal-500 to-cyan-600" />
          <KpiCard label="Faculty" value={String(data.staff)} sub="active members"
            icon={GraduationCap} gradient="bg-gradient-to-br from-amber-400 via-orange-500 to-red-500" />
          <KpiCard
            label="Treasury"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm p-7">
            ...
            <div className={`mt-5 rounded-2xl p-4 flex justify-between items-center border ${isDeficit ? 'bg-rose-50 border-rose-100' : 'bg-sky-50 border-sky-100'}`}>
              <div>
              ...
              <div className="relative w-14 h-14">
                <svg viewBox="0 0 44 44" className="w-full h-full -rotate-90">
                  <circle cx="22" cy="22" r="18" fill="none" stroke={isDeficit ? '#fecdd3' : '#bae6fd'} strokeWidth="4" />
                  <circle cx="22" cy="22" r="18" fill="none" stroke={isDeficit ? '#f43f5e' : '#0ea5e9'} strokeWidth="4"
                    strokeDasharray={`${donutFilled} 113`} strokeLinecap="round" />
                </svg>
                <span className={`absolute inset-0 flex items-center justify-center text-[9px] font-black ${isDeficit ? 'text-rose-600' : 'text-sky-600'}`}>
                  {balancePct}%
                </span>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm p-7">
              <div className="flex items-center gap-2.5 mb-6">
                ...
                <h2 className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-400">Class Density</h2>
              </div>
            ...
            <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm p-7">
              <div className="flex items-center gap-2.5 mb-6">
                ...
                <h2 className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-400">Students by Grade</h2>
              </div>
