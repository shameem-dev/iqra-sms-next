...
// ── Subjects ───────────────────────────────────────────────────────────────
const SUBJECTS = [
  { name: 'Arabic',         key: 'arabic',    full: 50 },
  { name: 'Malayalam',      key: 'malayalam', full: 50 },
  { name: 'IT',             key: 'it',        full: 50 },
  { name: 'G.K',            key: 'gk',        full: 50 },
  { name: 'Science',        key: 'science',   full: 50 },
  { name: 'Social Studies', key: 'social',    full: 50 },
  { name: 'Maths',          key: 'maths',     full: 50 },
  { name: 'English',        key: 'english',   full: 50 },
  { name: 'Hindi',          key: 'hindi',     full: 40 },
]
...
// ── Progress Report ────────────────────────────────────────────────────────
function buildPR(s: StudentWithMarks, ov: TCOverrides) {
  const m       = s.marks
  const markMap = m as unknown as Record<string, number>

  const totalObt  = SUBJECTS.reduce((a, sub) => a + (markMap[sub.key] ?? 0), 0)
  const totalFull = SUBJECTS.reduce((a, sub) => a + sub.full, 0)
  ...
  const subjectRows = SUBJECTS.map(sub => {
    ...
    return `<tr><td>${sub.name}</td><td>${sub.full}</td><td>${obt}</td><td class="gc">${calcGrade(obt, sub.full)}</td></tr>`
  }).join('')
...
table{width:100%;border-collapse:collapse;font-size:11.5px}
th{background:#1A2C6B;color:#fff;padding:5px 8px;text-align:left;font-weight:500;font-size:10.5px}
td{border:0.5px solid #ddd;padding:4px 8px}
...
<div class="page">
  ...
  <div class="student-info">
    ...
    <div class="info-row"><span class="info-label">Gender</span><span class="info-val">: ${s.gender}</span></div>
  </div>
  <div class="two-col">
    <div>
      <div class="section-title">Half Yearly Examination – 2025</div>
      <table>
        <thead><tr><th>Subject</th><th>Full</th><th>Obtained</th><th>Grade</th></tr></thead>
        <tbody>
          ${subjectRows}
          <tr class="total-row"><td>Total</td><td>${totalFull}</td><td>${totalObt}</td><td class="gc">${calcGrade(totalObt, totalFull)}</td></tr>
        </tbody>
      </table>
      <div class="att-box">Rank : ${ov.rank || '—'} &nbsp;|&nbsp; Attendance : ${ov.days_attended} / ${ov.school_days}</div>
    </div>
    <div>
      <div class="section-title">Moral Examination – 2025</div>
      <table>
        <thead><tr><th>Subject</th><th>Full</th><th>Obtained</th><th>Grade</th></tr></thead>
        <tbody>
          ${moralRows}
          <tr class="total-row"><td>Total</td><td>${moralFull}</td><td>${moralObt}</td><td class="gc">${calcGrade(moralObt, moralFull)}</td></tr>
        </tbody>
      </table>
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 0' }}>
      ...
      <div
        ...
        style={{
          ...
          background: value ? NAVY : BORDER, position: 'relative', transition: 'background 0.2s',
        }}
      >
        <div style={{
...
function TCEditPanel({ ov, setOv, onSave, saving }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {/* Personal details */}
      <div style={{ fontSize: 10, fontWeight: 700, color: NAVY, letterSpacing: '0.8px', textTransform: 'uppercase', borderBottom: `1px solid ${BORDER}`, paddingBottom: 4 }}>
        Personal Details
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        ...
        <SelectField label="Category"  value={ov.category}       onChange={set('category')}
          options={['General', 'OBC', 'SC', 'ST']} />
      </div>

      {/* Dates */}
      <div style={{ fontSize: 10, fontWeight: 700, color: NAVY, letterSpacing: '0.8px', textTransform: 'uppercase', borderBottom: `1px solid ${BORDER}`, paddingBottom: 4, marginTop: 4 }}>
        Dates
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
      ...
      <SelectField label="Conduct" value={ov.conduct} onChange={set('conduct')}
      ...
      <button
        onClick={onSave}
        disabled={saving}
        style={{ marginTop: 4, padding: '8px', fontSize: 12, fontWeight: 600, borderRadius: 7, border: `1px solid ${NAVY}`, background: NAVY, color: WHITE, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: saving ? 0.7 : 1, transition: 'opacity 0.15s' }}
      >
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        boxShadow: '0 24px 64px rgba(10,18,50,0.28)',
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: WHITE, letterSpacing: '0.3px' }}>
              Edit TC Fields
            </span>
          </div>
          <button
        ...
        <div style={{ overflowY: 'auto', padding: '18px 20px', flex: 1 }}>
          ...
          {saveMsg && (
            <div style={{
              ...
              color:      saveMsg.startsWith('✓') ? '#1D9E75' : '#c0392b',
              background: saveMsg.startsWith('✓') ? '#edfaf5' : '#fdf2f2',
              border: `1px solid ${saveMsg.startsWith('✓') ? '#a3e4cf' : '#f5c6cb'}`,
            }}>
              {saveMsg}
            </div>
          )}
        ...
        <div style={{
          ...
          <button
            onClick={onClose}
            style={{
              padding: '7px 18px', fontSize: 12, fontWeight: 500, borderRadius: 7,
              border: `1px solid ${BORDER}`, background: WHITE, color: TEXT_S,
...
export default function DocumentGenerator() {
  ...
  const docLabels = { tc: 'Transfer Certificate', cc: 'Conduct Certificate', pr: 'Progress Report' } as const
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif", display:'flex', gap:16, minHeight:600, background:LIGHT, borderRadius:12, padding:16 }}>
      ...
      <div style={{ width:210, flexShrink:0, display:'flex', flexDirection:'column', gap:8, background:WHITE, borderRadius:10, padding:12, border:`1px solid ${BORDER}`, boxShadow:'0 2px 8px rgba(26,44,107,0.06)' }}>
        ...
        <div style={{ fontSize:10, fontWeight:600, color:TEXT_S, marginTop: 8 }}>
            ...
            : filtered.map(s => {
                ...
                return (
                  <div key={s.id} onClick={() => selectStudent(s)} style={{ padding:'7px 9px', borderRadius:7, border:`1px solid ${active ? NAVY : BORDER}`, background:active ? NAVY : WHITE, cursor:'pointer', transition:'all 0.15s' }}>
                    ...
                    <div style={{ fontSize:10, color:active ? 'rgba(255,255,255,0.6)' : TEXT_S, marginTop:1 }}>
                      {s.standard} · #{s.admission_no}
                    </div>
                  </div>
                )
              })
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap', background:WHITE, borderRadius:10, padding:'9px 14px', border:`1px solid ${BORDER}`, boxShadow:'0 2px 8px rgba(26,44,107,0.06)' }}>
          ...
          {selected && docType === 'tc' && (
            <button
              ...
              style={{ padding:'5px 13px', fontSize:12, fontWeight:500, borderRadius:7, border:`1px solid ${GOLD}`, background:'#FDF3E3', color:GOLD, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:5 }}
            >
              Edit Fields
            </button>
          )}
