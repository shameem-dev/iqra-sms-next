...
const DEFAULT_STANDARD = 'LKG A'
  ...
  return allSubjectsMarks
    .map(student => {
      ...
      const subjectMarks = student.subjectMarks.map(sm => {
        const subj = subjects.find(s => s.id === sm.subject_id)
        return {
          subject_name: subj?.name ?? `Subject ${sm.subject_id}`,
          marks: sm.total,
          maxMarks: sm.maxTotal,
        }
      })
      return {
        ...
        admission_no: student.admission_no,
        gender: student.gender,
        total, maxTotal, percent, subjectMarks,
      }
    })
...
function getExamToppers(field: keyof Omit<MarkFormData, 'id'>) {
  ...
  return allSubjectsMarks
    .map(student => {
      ...
      const subjectMarks = student.subjectMarks.map(subjectMark => {
        const subject = subjects.find(s => s.id === subjectMark.subject_id)
        const markVal = subjectMark.marks[field] ?? null
        const maxVal  = subject ? ((subject[`max_${field}` as keyof Subject] as number) || 0) : 0
        total    += markVal || 0
        maxTotal += maxVal
        return {
